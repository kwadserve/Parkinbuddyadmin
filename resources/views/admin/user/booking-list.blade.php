...
<div class="grid grid-cols-12 gap-6 mt-5">
                 ...
                 ?>
                    <tr class="intro-x">
                        ...
                        <td class="text-center">
                            ...
                            <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5">{{$parkingDetails->city}}, {{$parkingDetails->state}}</div>
                        </td>
                        <td class="text-center">
                        ...
                        <td class="text-center">
                            <div class="whitespace-nowrap"><?=date_format(date_create($booking->start_time), "h:m:s a")?></div>
                            <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5"><?=date_format(date_create($booking->start_time), "d-m-Y")?></div>
                        </td>
                        <td class="text-center">
                            <div class="whitespace-nowrap"><?=date_format(date_create($booking->end_time), "h:m:s a")?></div>
                            <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5"><?=date_format(date_create($booking->end_time), "d-m-Y")?></div>
                        </td>
                        ...
                        <td class="text-center">
                            @if($booking->entry_time != null)
                                <div class="whitespace-nowrap"><?=date_format(date_create($booking->entry_time), "h:m:s a")?></div>
                        ...
                        <td class="text-center">
                            @if($booking->exit_time != null)
                                <div class="whitespace-nowrap"><?=date_format(date_create($booking->exit_time), "h:m:s a")?></div>
                                <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5">
                        ...
                        <td class="w-40 text-center font-medium whitespace-nowrap">₹ {{$booking->cash_collection}}</td>
                        <td class="text-center">
                            <?php $paymentIds = json_decode($booking->payment_ids); ?>
                            @if($paymentIds)
                            @foreach ($paymentIds as $payId)
                            <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5">{{$payId}}</div>
                            @endforeach
                            @endif
                        </td>
                        <td class="w-40 text-center text-primary whitespace-nowrap">{{$booking->parking_code}}</td>
                        <td class="text-center">
                            <div class="whitespace-nowrap"><?=date_format(date_create($booking->created_at), "h:m:s a")?></div>
                            <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5"><?=date_format(date_create($booking->created_at), "d-m-Y")?></div>
                        </td>
                        <td class="text-center">
                            <div class="whitespace-nowrap"><?=date_format(date_create($booking->updated_at), "h:m:s a")?></div>
                            <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5"><?=date_format(date_create($booking->updated_at), "d-m-Y")?></div>
                        </td> 
                    </tr>
                @endforeach
            @else
    ...
    @if($bookingData->count() > 0)
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center" id="userBook">
