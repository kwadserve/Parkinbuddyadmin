                ...
                @if ($userPassData->count() > 0)
                    @foreach ($userPassData as $userPass)
                        ...
                        <tr class="intro-x">
                            ...
                            <td class="w-40 text-center !py-4 whitespace-nowrap text-primary">{{ $userPass->title }}
                            </td>
                            ...
                            <td class="text-center">
                                <div class="whitespace-nowrap">
                                    <?= date_format(date_create($userPass->start_time), 'd-m-Y') ?></div>
                            </td>
                            <td class="text-center">
                                <div class="whitespace-nowrap">
                                    <?= date_format(date_create($userPass->end_time), 'd-m-Y') ?></div>
                            </td>
                            <td class="text-center">
                                <div class="whitespace-nowrap">{{ $userPass->remaining_hours }}</div>
                            </td>
                            <td class="w-40 text-center font-medium whitespace-nowrap">₹ {{ $userPass->amount }}</td>
                            <td class="text-center">
                                <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5">{{ $userPass->payment_id }}
                                </div>
                                <!-- <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5">pay_MFKXGtRAHwyGNL</div> -->
                            </td>
                            <td class="text-center">
                                <div class="whitespace-nowrap">
                                    <?= date_format(date_create($userPass->created_at), 'h:m:s a') ?></div>
                                <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5">
                                    <?= date_format(date_create($userPass->created_at), 'd-m-Y') ?></div>
                            </td>
                            <td class="text-center">
                                <div class="whitespace-nowrap">
                                    <?= date_format(date_create($userPass->updated_at), 'h:m:s a') ?></div>
                                <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5">
                                    <?= date_format(date_create($userPass->updated_at), 'd-m-Y') ?></div>
                            </td>
                        </tr>
                    @endforeach
                @else
