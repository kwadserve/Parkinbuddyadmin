            ...
            @if($parkingsData->count() > 0)
                @foreach ($parkingsData as $parking)
                    ...
                    <tr class="intro-x">
                        ...
                        <td class="text-center"><div class="whitespace-nowrap">{{$parking->pin_code}}</div></td>
                        <td class="text-center"><div class="whitespace-nowrap">{{$parking->latitude}}</div></td>
                        <td class="text-center"><div class="whitespace-nowrap">{{$parking->longitude}}</div></td>
                        <td class="text-center"><div class="whitespace-nowrap">
                            @if($operatorName && $operatorName->name != null) {{$operatorName->name}}
                            @else {{'-'}}
                            @endif</div></td>
                        <td class="text-center"><div class="whitespace-nowrap"> @if($managerName && $managerName->name != null) {{$managerName->name}}
                            @else {{'-'}}
                            @endif</div></td>
                        <td class="text-center">
                            <div class="whitespace-nowrap"><?=date_format(date_create($parking->created_at), "h:m:s a")?></div>
                            <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5"><?=date_format(date_create($parking->created_at), "d-m-Y")?></div>
                        </td>
                        <td class="text-center">
                            <div class="whitespace-nowrap"><?=date_format(date_create($parking->updated_at), "h:m:s a")?></div>
                            <div class="text-slate-500 text-xs whitespace-nowrap mt-0.5"><?=date_format(date_create($parking->updated_at), "d-m-Y")?></div>
                        </td> 
                    </tr>
                @endforeach
            @else
